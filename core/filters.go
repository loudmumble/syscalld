...
type SensorFilter struct {
	...
	// ExcludePIDs excludes these PIDs from monitoring. Works independently of TargetPIDs.
...
// ExcludePIDs is checked first and takes precedence over TargetPIDs.
func (f *SensorFilter) MatchesPID(pid int) bool {
	if len(f.ExcludePIDs) > 0 {
